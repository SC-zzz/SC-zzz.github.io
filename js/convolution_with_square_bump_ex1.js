function new_convolution_ex1() { 
// f = square bump
  let L = parseFloat(document.getElementById("L_ex1").value);  
  let h = 0.01;
  let t_start = -3;
  let t_fin = 3;
  ...
  let gArray = tArray.map(function(t) {
    if (t >= -1 - 1/(2 * L) && t <= 1 + 1/(2 * L)) {
      return L * (Math.min(t + 1/(2 * L), 1) - Math.max(t - 1/(2 * L), -1));}
    else {
      return 0;}  
    });
  ...
  // Define Data
  if (document.getElementById("hideUL_ex1").checked) {
    var data = [{
		x:tArray,
		y:fArray,
		mode:"lines",
  ...
  Plotly.newPlot("convEx1Plot", data, layout);
}
