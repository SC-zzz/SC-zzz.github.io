...
function drawSpot(board, x, y, v, rangeVal,h) {
  let colFunc = "hsl(" + (240 - 180 * v/rangeVal) + ", 100%, 80%)"; 


	let cu = board.create('curve', 
		[[x-h/2, x+h/2, x+h/2, x-h/2, x-h/2],
		 [y-h/2, y-h/2, y+h/2, y+h/2, y-h/2]],
		{strokeWidth:0, fillColor:colFunc, fillOpacity:0.5, highlightFillColor:colFunc, highlightFillOpacity:0.5});
...
function createMyHeatMap(board, xCoord, yCoord, arr2D, xlen, ylen, h) {
  let i, j, heat = [];
  let maxVal = math.max(arr2D);     
  let minVal = math.min(arr2D);  
  let rangeVal = maxVal-minVal;           
  board.suspendUpdate();
  for (i=0; i<xlen; i++) {	
    //heat[i] = [];
      for (j=0; j<ylen; j++) {	
		drawSpot(board, xCoord[i], yCoord[j], arr2D[i][j]-minVal, rangeVal, h);
	  }
  }
